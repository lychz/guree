...
describe("Switch 组件", () => {
  ...
  it("测试 Switch 组件 checkedChildren 参数", () => {
    const sw = renderer
      .create(<Switch checkedChildren={"checked"}></Switch>)
      .toJSON();
    expect(sw).toMatchSnapshot();
  });
  ...
  it("测试 Switch 组件 unCheckedChildren 参数", () => {
    const sw = renderer
      .create(<Switch unCheckedChildren={"unchecked"}></Switch>)
      .toJSON();
    expect(sw).toMatchSnapshot();
  });
  ...
  it("测试 Switch 组件 onChange 参数", () => {
    ...
    sw.update();
    expect(sw.find(".guree-switch").hasClass("guree-switch-checked")).toEqual(
      true
    );
    sw.simulate("click");
  ...
  it("测试 Switch 组件 checked 不为 undefined 状态下的点击事件", () => {
    ...
    sw1.update();
    sw2.update();
    expect(sw1.find(".guree-switch").hasClass("guree-switch-checked")).toEqual(
      true
    );
    expect(sw2.find(".guree-switch").hasClass("guree-switch-checked")).toEqual(
      false
    );
  });
});
